...
export default function CardImport() {
  const { socket, lobbyState } = useGame();
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      if (data.blackCards.length < 5) throw new Error("Need at least 5 black cards");
      if (data.whiteCards.length < 20) throw new Error("Need at least 20 white cards");

      const badBlack = data.blackCards.find((c) => typeof c !== "string");
      const badWhite = data.whiteCards.find((c) => typeof c !== "string");
      if (badBlack || badWhite) throw new Error("All cards must be strings");

      socket.emit("cards:upload-cards", {
  ...
  return (
    <div className="bg-surface rounded-lg p-4 space-y-3">
      ...
      {status && (
        <p className={`text-xs ${status.type === "success" ? "text-green-400" : "text-red-400"}`}>{status.msg}</p>
      )}
      {deckInfo && (
        <p className="text-xs text-gray-500">{deckInfo.black} black cards, {deckInfo.white} white cards</p>
      )}
      <details className="text-xs text-gray-600">
        <summary className="cursor-pointer hover:text-gray-400 transition">JSON format</summary>
